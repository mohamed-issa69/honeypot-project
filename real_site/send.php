...
<script src="js/javascript.js"></script> 
	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
	    ...
	    $rphone = $_POST['rphone'];
	    $pin = rand(1000,9999);
	    $trxid = 'EGP'.rand(1000,9999).rand(1000,9999);
    ...
    $query = "INSERT INTO money(sname,sphone,amount,rname,rphone,pin,trxid,sentby,type) VALUES('$sname','$sphone','$amount','$rname','$rphone','$pin','$trxid','$sentby','sent')";
    	...
    	if ($inserted_rows) {
            ...
            // SMS Sending 
					...
					$sender = 'GET MONEY';
					$message = 'You Have Received '.$amount. ' EGP from '.$sphone.' , Your Pin: '.$pin.' And Trx ID: '.$trxid.' .';
				 
					$response = $textlocal->sendSms($numbers, $message, $sender);
					//print_r($response);
					$numbers = array($sphone);
					$sender = 'GET MONEY';
					$message = 'Sent Money ' .$amount.' EGP to '.$rphone.' Successful. TrxID:'.$trxid. '.';
